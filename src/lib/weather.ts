...
export const FALLBACK_WEATHER = { label: "맑음", emoji: "☀️" } as const;
...
export function labelWeatherCode(code: number): { label: string; emoji: string } {
...
export async function fetchSeoulWeather(): Promise<WeatherNow> {
  ...
  return { ...labelWeatherCode(code), temperature };
}
